...
| **Sync path guard** | ✅ a shell step in the generated `sync-reusable.yml` fails the job before either commit strategy if the sync touched a path outside the fixed allowlist; `agconf verify-paths` applies the same list locally | ➖ nothing commits — the store is machine-local and backs up anything it overwrites (INV-4) | ➖ agconf writes nothing into a repo; the harness installs the plugin |
