...
- `propose.ts` - Downstream→canonical proposals (the reverse of `sync`). `detectProposedChanges` maps modified managed files to canonical paths, `detectNewContent` discovers unmanaged local content, `applyProposedChanges` commits/pushes/opens the PR. Every proposed file is reconciled against canonical HEAD before shipping — see [Propose Rebasing](#propose-rebasing). **Repo scope only:** both detectors start from `readLockfile(targetDir)` and scan repo-relative paths, and `proposeCommand` has no `--scope`; there is no way to propose from a user-scope projection. Before adding one, read [Distribution Scopes §17.1](cli/docs/DISTRIBUTION_SCOPES.md#171-propose-at-user-scope-the-gap) — the path mapping and rebase carry over unchanged at `targetDir = homeDir`, but the instructions block (which lives in `~/.claude/CLAUDE.md`, not `<dir>/AGENTS.md`), `--new` discovery over the developer's personal skills, and the git provenance in the PR body all need scope-aware handling.
