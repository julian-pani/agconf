...
- `user-scope.ts` - **User-scope sync** (`sync --scope user`). Projects only the canonical **global instructions block** into a developer's per-user harness files (`~/.claude/CLAUDE.md`, `~/.codex/AGENTS.md`) via a marker upsert (`projectGlobalBlock`) that preserves all other content, so company standards live once per machine instead of in every repo. The company block is pure canonical (reuses `buildGlobalBlock`/`computeGlobalBlockHash`, so `checkUserScope` reuses `hasGlobalBlockChanges`). A git-tracked store at `~/.agconf/` holds the user lockfile (via `writeLockfile(homeDir, …)`), a `global.md` mirror (readable diffs), a never-overwritten `USER.md` (the personal layer — imported on Claude via `@~/.agconf/USER.md`, referenced by note on Codex), and timestamped `backups/` of any drifted/unmanaged file a projection would overwrite (rotated to the last 10). All paths derive from an injectable `homeDir` for testability. Command layer: `commands/user-scope.ts`. Instructions/rules-only (skills/agents/mcps go via plugins, per the delivery map). See [Distribution Scopes](cli/docs/DISTRIBUTION_SCOPES.md).

### Commands (`cli/src/commands/`)
Commands: init, sync (with `--scope repo|user`), check (with `--hook` for pre-commit: branch-aware exit — block on master/main, warn on feature branches; and `--scope repo|user`), compile, propose, upgrade-cli (with `--package-manager` option), canonical (init), config, completion
