...
pnpm start <command>      # Run without building (e.g., pnpm start init --help). No `--`: pnpm 10 forwards it, and commander then reads every option as a stray positional
pnpm dev                  # Watch mode
...
- `verify-paths.ts` - **Sync path guard**, local half. `agconf verify-paths` matches every working-tree change against a fixed, coarse allowlist: `BASE_ALLOWED_PATHS` (`AGENTS.md`, `CLAUDE.md`, `.agconf/**`, `.claude/**`, `.codex/**`, `.agents/**`, `.pre-commit-config.yaml`/`.yml`) plus the two workflow files named after the marker prefix (`getManagedWorkflowFilenames`). Nothing widens it — there is no config knob, on purpose. **The copy that enforces in CI is not this module**: it is a shell step generated into `sync-reusable.yml` by `generateSyncWorkflow` in `commands/canonical.ts`, which spells the allowlist out as a literal bash array so a downstream reviewer can audit what a sync may write without knowing agconf. That is the whole point of inlining it; do not "simplify" it into an `agconf verify-paths` call. `tests/integration/sync-workflow-guard.test.ts` extracts that step's script out of the generated YAML, executes it against a real git repo per case, and pins both the array **and** the matching behavior (a parity table) against this module. Hard-won invariants in that shell, each of which shipped broken once — **no backticks** (inside a double-quoted shell string they are command substitution, and the failure handler executed both the violating path and `agconf sync`); **`git status` captured to a file, never a pipeline or process substitution** (under `set -e` those swallow a git failure, leaving an empty loop and a green step — the guard's one job is failing closed); **`-uall -z`** (the default collapses a new directory to `dir/`, and porcelain quotes any path containing a space, which turned a legitimate `.claude/skills/my skill/` into a violation); and the bash local is `dir_prefix`, not `prefix`, because `prefix` is also `generateSyncWorkflow`'s TS parameter and a missed backslash would silently interpolate it with no compile error. Both copies support exactly two pattern shapes (a `dir/**` prefix or an exact path). The guard runs before the strategy branch, so it covers `pr` as well as `direct`, and fails the job with nothing committed.
...
Commands: init (with `--scope repo|user`), sync (with `--scope repo|user`), check (with `--hook` for pre-commit: branch-aware exit — block on master/main, warn on feature branches; and `--scope repo|user`), compile, propose (with `--scope repo|user`), verify-paths, upgrade-cli (with `--package-manager` option), canonical (init), config, completion, session-check (with `--install-hook` and `--hook`), autosync (with `--install`/`--uninstall`/`--enable`/`--disable`/`--force`/`--trigger`)
...
> `init --scope user` is the guided front door to user-scope setup, in
> `commands/init-user-scope.ts` (its own module, not `user-scope.ts`, because it
> imports `enableAutosync` from `commands/autosync.ts`, which already imports
> `user-scope.ts`). It is a **thin orchestrator**: it prompts for the source
> (recovered from the store lockfile on a re-run), the targets, and auto-sync,
> then delegates to `runUserScopeSync`, followed by either `enableAutosync` (which
> installs the SessionStart hook *and* writes the opt-in config) or, when auto-sync
> is declined, `installStoreHooks` alone. Hooks are installed *after* the sync
> because `resolveHookTargets` reads the targets out of the store lockfile. Auto-sync
> defaults **on** here (`--no-autosync` opts out); the config-presence opt-in that
> guards `autosync` against activating on upgrade is already satisfied by the
> explicit `init` — but a machine where auto-sync was deliberately disabled keeps
> that preference, and declining it is *recorded* (`setAutosyncEnabled(false)`,
> written before the hook install and regardless of whether one was already
> enabled), so the CLI never reports a state it did not write and a later `--yes`
> run cannot read the decline as "never configured" and switch it back on.
> `--yes` keeps it scriptable and errors (rather than prompting) when there is no
> source. A failed hook install is surfaced (non-zero exit, no success outro)
> instead of being followed by "Done.". Result rendering is shared with
> `sync --scope user` via the exported `printUserSyncResult`.
>
...
>
> `session-check` (`commands/session-check.ts`, core `core/session-check.ts`) is an **advisory, exit-0** cross-scope duplication + user-scope integrity check meant to run at session start (via a `SessionStart` hook installed by `--install-hook` for each target the user store was synced to — Claude Code → `~/.claude/settings.json`, Codex → `~/.codex/hooks.json`; `installSessionStartHooks`/`resolveHookTargets` in `core/session-check.ts`, with `getCodexHooksState` warning when Codex's stable-by-default `hooks` feature was explicitly disabled). Because `resolveHookTargets` is only snapshotted at install time (never re-reconciled), the advisory path also runs `findMissingHookTargets` — comparing the store's targets against the hooks actually present in those two files — and nudges the developer to re-run `--install-hook` for any target the store gained afterwards (e.g. a Claude-only install followed by `sync --scope user --target claude,codex`); it never throws, and an unreadable/malformed config counts as "installed" so it never nags about a file `--install-hook` couldn't fix. Detection is **identity-based, never content-equality**: **instructions** (a single block) flag on presence in ≥2 scopes (repo lockfile vs `~/.agconf` lockfile), the hash only annotating identical vs divergent; **skills/rules/agents** flag only the specific objects present in BOTH scopes (real overlap — repo skill X + user skill Y is not a collision). All notes print under one header (`printNotes`) that instructs the coding agent to **relay them to the human developer at the start of its next reply** and not act on them itself — the agent is the developer's only channel to this output, so purely context-framed text ends up read and silently dropped. Output goes to stdout so the hook injects it into context; it never throws. With `--hook` (what `--install-hook` writes into the hook command) the notes are emitted as the SessionStart wire envelope — `{hookSpecificOutput:{hookEventName,additionalContext}}` — because Codex validates a hook's stdout against its `session-start.command.output` schema and drops the output (`hook: SessionStart Failed`) for anything else; Claude Code accepts the same envelope, so one command serves both. The envelope is emitted even when there is nothing to report (empty `additionalContext`) — empty stdout is not valid hook output either — and `--quiet` suppresses the notes entirely. Without `--hook` the text is human-framed on a TTY and relay-framed otherwise (so hooks installed before the flag keep working). Hook entries are classified by `sessionStartHookState` (`current`/`stale`/`absent`): `--install-hook` rewrites only the exact command agconf wrote (`upgradeLegacyHookCommands`), warns about a customized `stale` command instead of editing someone else's command line, never adds a second entry beside an existing one, and `stale` counts as not-installed for `findMissingHookTargets` so the nudge reaches pre-`--hook` installs. When user scope is set up AND auto-sync is installed+enabled, it also kicks off the detached background `autosync` (`maybeStartBackgroundAutosync`) and then runs a cheap, bounded **freshness probe** (`probeUserScopeFreshness` — a version lookup, no clone); if the store is behind canonical's latest release it prints a note recommending the developer restart the session to load the update (the current session can't reload already-read context; the background refresh makes the restart current).
...
Reconciliation runs **once per canonical path**, not once per downstream file.
A skill synced to several targets has one copy per target
(`.claude/skills/X/…`, `.agents/skills/X/…`) but a single canonical home, so
`collapseByCanonicalPath` folds the candidates before anything is reconciled:
copies that agree become one proposal, copies that disagree raise
`DivergentCopiesError` and abort. `--override` deliberately does **not** resolve
that — it picks local over canonical, and here both sides are local. The asset
scan therefore walks **every** target rather than stopping at the first one that
has the skill dir; an asset edited only in a later target's copy must still be
proposed. (`collapseInstructionFiles` is the user-scope analogue for the
instructions block, aborting earlier and with its own error.)
...
- `cli/README.md`
- `cli/docs/` guides that reference the command (user-scope commands are documented in `cli/docs/USER_SCOPE.md`)
