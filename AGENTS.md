...
- `autosync.ts` - **User-scope auto-sync** (`agconf autosync`). Keeps the per-user store fresh automatically: enabled by default (safe because the store is git-tracked + backed up), disabled via `~/.agconf/config.yaml` (`autosync.enabled: false`, `UserScopeConfigSchema`/`loadUserScopeConfig`). Two triggers, one runner: the SessionStart hook calls `maybeStartBackgroundAutosync` (detached, throttled) so it returns instantly; a `crontab` entry (`installCron`/`buildCrontab`, idempotent by the `# agconf-autosync` marker, mac/Linux) runs it every `interval_minutes` with `--force` so scheduled runs bypass the session-start throttle (whose window equals the interval). The crontab read only treats "no crontab"/missing-binary as empty — any other failure propagates rather than risking a write that wipes the user's entries; `--install` therefore treats a crontab failure as a non-fatal warning (session-start autosync still works) and `--uninstall` points at the real off-switch (`autosync.enabled: false`), leaving the shared SessionStart hook in place. Cheap when current — `runUserScopeSync({skipIfUpToDate})` skips the clone when the store is at/ahead of latest. Throttle STATE is `~/.agconf/autosync-state.json` (`last_attempt`, config-vs-state split); runs append to `~/.agconf/logs/autosync.log` (rotated). Always best-effort/exit-0. Command layer: `commands/autosync.ts` (`--install`/`--uninstall`/`--force`/`--trigger`).
